isr_0:
    ...
    push 0          ; interrupt_handler procedure's arguments
    jmp isr_basic
...
isr_32:
	...
	; esp is initialially set to 07c00h by BIOS
	; push necessary context to stack, to be used by scheduler
	pusha ; push all general purpose registers' value to stack in order EAX, ECX, EDX EBX, ESP (the value prior to executing the PUSHA instruction), EBP, ESI, and EDI
	mov eax, [esp + 32] ; eip before this interrupt is stored at the stack top
    push eax
	call scheduler
	; notify master PIC
	mov al, 0x20
	...
	add esp, 40d
	push run_next_process ; overwrite return address as run_next_process
...
isr_basic:
    call interrupt_handler
    pop eax                 ; eliminating the argument that we have pushed for interrupt_handler; Equivalently, 'add esp, 4'
    sti
    iret
...
irq_basic:
    ...
    ; tell PCI that the IRQ it sent has been handled, send end of interrupt (EOI) command (0x20)
	mov al, 0x20
	out 0x20, al
    ; but for the slave PIC this command should be sent only after the IRQs of slave PIC are handled, that is, interrupt number 40 till 48
	cmp byte [esp], 40d
	jnge irq_basic_end
	mov al, 0x20
	out 0xa0, al
