...
int get_address_by_filename(char* filename)
{
    ...
    metadata_t *curr_file = load_metadata(curr_file_address);
    while (1)
    {
        ...
        curr_file = load_metadata(curr_file_address);
    }
    return 0;
}
...
void delete_file( char *filename )
{
    ...
    metadata_t *curr_file = read_disk(curr_file_address);
    ...
    {
        update_base_block(curr_file->next_file_address, base_block->tail);
    }
    else
    {
        int prev_file_address = get_prev_file_address(curr_file_address);
        metadata_t *prev_file = read_disk(prev_file_address);
        prev_file->next_file_address = curr_file->next_file_address;
